...
export const GameAds = () => {
	...
	useEffect(() => {
		fetch(`${baseApiUrl}/games/${gameId}/ads`)
			.then((res) => res.json())
	...
	return (
		<div className="max-w-[1344px] mx-auto flex flex-col items-center m-20 px-20">
			...
			<h1 className="text-4xl text-white font-bold mt-20 mb-8 text-center">
				Veja os últimos anúncios
			</h1>
			<div ref={sliderRef} className="flex flex-wrap gap-5 lg:flex-row">
				{ads.length > 0 ? (
					ads.map((ad) => <AdCard key={ad.id} data={ad} />)
				) : (
					<Alert />
				)}
			</div>
